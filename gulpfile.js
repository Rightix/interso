...
gulp.task('watch', function() {
    ...
    gulp.watch('app/plain/sass/*.sass', gulp.parallel('sass')); // Наблюдение за sass файлами в папке sass
    gulp.watch('app/plain/css/*.css', gulp.parallel('css')); // Наблюдение за sass файлами в папке sass
    // gulp.watch("app/**/*.html", ['include']);
    gulp.watch(['app/*.html', 'app/**/*.html'], gulp.parallel('include')).on('change', browserSync.reload); // Наблюдение за HTML файлами в корне проекта
    gulp.watch('app/plain/js/*.js',gulp.parallel('scripts'));  // Наблюдение за JS файлами в папке js
    gulp.watch('app/plain/svg/*.svg', gulp.parallel('svg'));
    gulp.watch('app/img/*', gulp.parallel('tinypng'));
    // gulp.watch('dist/plain/css/*.css');
    // livereload.listen();
});

gulp.task('clean', function() {
    return del.sync('dist/*.html'); // Удаляем папку dist перед сборкой
});
...
gulp.task('build', gulp.parallel('sass', 'scripts'), function() {
...
gulp.task('default', gulp.parallel('watch', 'css', 'scripts', 'include', 'server', 'svg', 'tinypng'));
